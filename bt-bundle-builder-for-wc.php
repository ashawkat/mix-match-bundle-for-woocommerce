...
function mmb_add_bundle_to_cart() {
    ...
    wp_send_json_success( __( 'Bundle added to session', 'bt-bundle-builder-for-wc' ) );
}
    ...
    try {
        ...
        if ( ! empty( $products_json ) ) {
            ...
            if ( empty( $products ) || ! is_array( $products ) ) {
                wp_send_json_error( __( 'Invalid products data', 'bt-bundle-builder-for-wc' ) );
            }
            ...
            foreach ( $grouped_products as $grouped ) {
                ...
                if ( $product_id < 1 ) {
                    $failed_items[] = __( 'Invalid product ID', 'bt-bundle-builder-for-wc' );
                    continue;
                }
                ...
                if ( ! $product || ! $product->is_purchasable() ) {
                    $failed_items[] = sprintf(
                        ...
                        __( 'Product cannot be purchased: %s', 'bt-bundle-builder-for-wc' ),
                ...
                if ( $cart_item_key ) {
                    ...
                    $failed_items[] = sprintf(
                        ...
                        __( 'Failed to add product %d', 'bt-bundle-builder-for-wc' ),
                        $product_id
                    );
                }
            }
            ...
            if ( empty( $added_items ) ) {
                wp_send_json_error( [
                    'message' => __( 'Failed to add any products to cart', 'bt-bundle-builder-for-wc' ),
                    'errors' => $failed_items,
                ] );
            }
        ...
        if ( empty( $_POST['products'] ) ) {
            ...
            if ( $product_id < 1 ) {
                wp_send_json_error( __( 'Invalid product ID', 'bt-bundle-builder-for-wc' ) );
            }
            ...
            if ( $cart_item_key ) {
                ...
                wp_send_json_error( __( 'Failed to add product to cart', 'bt-bundle-builder-for-wc' ) );
            }
        }
    } catch ( Exception $e ) {
        ...
        wp_send_json_error( [
            'message' => __( 'An error occurred while adding products to cart', 'bt-bundle-builder-for-wc' ),
...
function mmb_init_plugin() {
    ...
    if ( mmb_check_woocommerce_dependency() ) {
        // Initialize plugin
        BT_Bundle_Builder::get_instance();
    }
}
        ...
        add_action( 'admin_notices', function() use ( $result ) {
            printf(
                ...
                sprintf(
                    ...
                    esc_html__( 'Bundle coupon cleanup completed. Deleted: %1$d, Kept: %2$d', 'bt-bundle-builder-for-wc' ),
...
function mmb_get_date_range_label( $date_range, $start_timestamp, $end_timestamp ) {
    switch ( $date_range ) {
        case '7days':
            return __( 'Last 7 Days', 'bt-bundle-builder-for-wc' );
        case '30days':
            return __( 'Last 30 Days', 'bt-bundle-builder-for-wc' );
        case '90days':
            return __( 'Last 90 Days', 'bt-bundle-builder-for-wc' );
        case 'this_month':
            return __( 'This Month', 'bt-bundle-builder-for-wc' );
        case 'last_month':
            return __( 'Last Month', 'bt-bundle-builder-for-wc' );
        case 'this_quarter':
            return __( 'This Quarter', 'bt-bundle-builder-for-wc' );
        case 'last_quarter':
            return __( 'Last Quarter', 'bt-bundle-builder-for-wc' );
        case 'this_year':
            return __( 'This Year', 'bt-bundle-builder-for-wc' );
        case 'last_year':
            return __( 'Last Year', 'bt-bundle-builder-for-wc' );
        case 'custom':
            return sprintf(
                    /* translators: 1: start date, 2: end date */
                    __( 'Custom: %1$s to %2$s', 'bt-bundle-builder-for-wc' ),
                    gmdate( 'M j, Y', $start_timestamp ),
                    gmdate( 'M j, Y', $end_timestamp )
                );
        default:
            return __( 'Custom Range', 'bt-bundle-builder-for-wc' );
    }
}
...
function mmb_get_conversion_analytics( $start_date, $end_date ) {
    ...
    return array(
        ...
        'note' => ! $table_exists ? __( 'Install WP Statistics plugin for detailed analytics', 'bt-bundle-builder-for-wc' ) : ''
    );
}
...
function mmb_get_bundle_performance_analytics( $start_date, $end_date ) {
    ...
    foreach ( $bundle_data as $bundle ) {
        ...
        $bundle_usage[] = array(
            'id' => $bundle->id,
            'name' => isset( $bundle->name ) ? $bundle->name : __( 'Unnamed Bundle', 'bt-bundle-builder-for-wc' ),
